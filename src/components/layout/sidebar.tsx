...
export const Sidebar = () => {
  ...
  return (
    <aside
      className={cn(
        "sticky top-0 hidden h-screen shrink-0 border-r border-white/10 bg-slate-950/90 p-3 backdrop-blur lg:flex lg:flex-col",
    ...
    >
      <div className="mb-5 flex items-center justify-between px-2 py-3">
        <div className={cn("overflow-hidden transition-all", sidebarCollapsed ? "w-0" : "w-auto")}>
          <p className="text-xs uppercase tracking-[0.24em] text-cyan-300">E-COM-OS</p>
          <p className="text-sm text-slate-400">Dropshipping Control</p>
        </div>
        <button
          type="button"
          onClick={toggleSidebar}
          className="rounded-lg border border-white/10 bg-slate-900 p-2 text-slate-300 hover:text-white"
